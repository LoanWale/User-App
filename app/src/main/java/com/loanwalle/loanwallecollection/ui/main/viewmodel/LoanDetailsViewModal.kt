...
class LoanDetailsViewModal(
    app: Application,
    private val appRepository: AppRepository
) : AndroidViewModel(app) {

    private val _userProfileResponse = MutableLiveData<Event<Resource<LoanDetailsResponse>>>()
    val userProfileResponse: LiveData<Event<Resource<LoanDetailsResponse>>> = _userProfileResponse


    fun userProfile(body: LoanDetailsReq) = viewModelScope.launch {
        callProfile(body)
    }


    private suspend fun callProfile(body: LoanDetailsReq) {
        _userProfileResponse.postValue(Event(Resource.Loading()))
        try {
            if (Utils.hasInternetConnection(getApplication<MyApplication>())) {
                val response = appRepository.getLoanDeatils(body)
    ...
    private fun handlePicsResponse(response: Response<LoanDetailsResponse>): Event<Resource<LoanDetailsResponse>>? {
